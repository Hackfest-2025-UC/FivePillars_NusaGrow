...
@section('content')
    ...
    <form action="{{ route('products.update', $produk->id_produk) }}" method="POST" enctype="multipart/form-data"
        ...
        <div class="flex justify-around w-full items-center gap-5">
            <div class="w-full">
                ...
                <input type="text"
                    ...
                    placeholder="Satu Wadah Getah Karet" name="nama_produk" required value="{{ $produk->nama_produk }}" />
            </div>
            <div class="w-full">
                ...
                <select
                    ...
                    <option selected {{ $produk->kategori_produk == 'Pertanian' ? 'selected' : '' }}>Pertanian</option>
                    <option {{ $produk->kategori_produk == 'Elektronik' ? 'selected' : '' }}>Elektronik</option>
                    <option {{ $produk->kategori_produk == 'Peralatan Rumah Tangga' ? 'selected' : '' }}>Peralatan Rumah
                        Tangga</option>
                    <option {{ $produk->kategori_produk == 'Bahan Makanan' ? 'selected' : '' }}>Bahan Makanan</option>
                </select>
            </div>
            <div class="w-full">
                ...
                <input type="number"
                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 placeholder-gray-400"
                    placeholder="1000000" name="harga" required value="{{ $produk->harga_produk }}" />
            </div>
        </div>
        <div class="w-full flex items-start gap-5">
            ...
            <div class="w-2/3">
                ...
                <textarea id="message" rows="1"
                    ...
                    placeholder="Masukkan deskripsi disini..." name="deskripsi">{{ $produk->deskripsi_produk }}</textarea>
            </div>
            <div class="w-1/3">
                <div class="w-full">
                    ...
                    <input type="text"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                        placeholder="Jl. Pemuda No. 1" name="alamat" required value="{{ $produk->alamat }}" />
                </div>
            </div>
        </div>
            ...
            <div class="flex justify-around w-full items-center gap-5">
                <div class="w-full">
                    ...
                    <input type="number" id="inpLatitude" name="latitude"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                        placeholder="-6.123456" readonly required value="{{ $produk->latitude }}" />
                </div>
                <div class="w-full">
                    ...
                    <input type="number" id="inpLongitude" name="longitude"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                        placeholder="106.123456" readonly required value="{{ $produk->longitude }}" />
                </div>
            </div>
        </div>
        <div class="flex justify-end">
            <button type="submit"
                class="w-fit text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-right">Update</button>
        </div>
    </form>
    ...
    @pushonce('scripts')
        ...
        <script>
            var map = L.map('map').setView([{{ $produk->latitude }}, {{ $produk->longitude }}], 12);

            L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
                attribution: '&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors'
            }).addTo(map);

            var lastMarker = L.marker([{{ $produk->latitude }}, {{ $produk->longitude }}]).addTo(map)
                .bindPopup("Latitude: {{ $produk->latitude }}<br>Longitude: {{ $produk->longitude }}").openPopup();
